...
import { ADMIN_COOKIE_NAME, SUPABASE_ACCESS_COOKIE_NAME, createSessionToken } from "@/lib/auth/session";
...
async function loginWithLocal(username, password) {
  ...
  return { id: "local-admin", username, provider: "local", accessToken: null };
}
...
export async function POST(request) {
  try {
    ...
    if (user.provider === "supabase" && user.accessToken) {
      response.cookies.set(SUPABASE_ACCESS_COOKIE_NAME, user.accessToken, {
        httpOnly: true,
        sameSite: "lax",
        secure: process.env.NODE_ENV === "production",
        path: "/",
